...
price_range = st.sidebar.slider('Price Range Selection', min_value=0, max_value=1500, value=(0,1000))
# price_sorting = st.sidebar.checkbox("Price low to high")
No_of_product = st.sidebar.slider('No. of Products Selection', min_value=0, max_value=10, value=(1))
...
#with row0_1:
logo = st.image("logo2.png", use_column_width=True) 
...
st.write(
    """
    Welcome to our "SpotDbuy" streamlit application!  Please enjoy shopping journey with remarkable experiences here!
    """)
...
def get_feature_vector(img):
    image = cv2.resize(img, (227, 227),3)
    feature_vector = basemodel.predict(image.reshape( 1, 227, 227,3))
...
basemodel=load_model('./vgg16_trsf.h5')
...
def yolo_v4(image, confidence_threshold=0.1, overlap_threshold=0.3):
    ...
    # FOR EACH DETECTED OBJECT, COMPUTE THE BOX AND FIND THE SCORE
...
def cal_sim(image):
    ...
    if 'Adidas' in yoloclass:
        for i in yoloclass:
            if i=='Short-T':
                for i in range(1841): #1842
                    v_shortT.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/shortT_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='Long-T':
                for i in range(529):
                    v_longT.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/longT_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='Polo-T':
                for i in range(385):
                    v_poloT.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/poloT_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='Hoodie':
                for i in range(1287):
                    v_hoodies.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/hoodies_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='Shorts':
                for i in range(1359):
                    v_shorts.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/shorts_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='Pants':
                for i in range(1450):
                    v_pants.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/pants_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='SportBra':
                for i in range(54):
                    v_sportbra.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/sportbra_f2_'+str(i)+'.txt', delimiter=','))*100)
            elif i=='Jacket':
                for i in range(614):
                    v_jacket.append(calculate_similarity(f1[0], np.loadtxt('./vectors2/jacket_f2_'+str(i)+'.txt', delimiter=','))*100)
...
if st.button("Click Me!"):
    ...
    #   'Wait a moment, We are detecting your image and make some recommendation for you ...'
        ...
        **Recommendations **
        ...
        # row3_1, row3_2 = st.beta_columns((2,2))
